...
<div class="content"><!--内容-->
    ...
    <div class="content-evaluate">
        <div>
            ...
            <div class="answer">
                ${sessionScope.answers[0]}
            </div>
        </div>
        <div>
            <h5>2.试卷分析（教学效果） （总成绩构成、成绩分布、考生掌握程度、考生错误率较高的知识点分布及原因）</h5>
           <div class="answer">
               ${sessionScope.answers[1]}
           </div>
        </div>
        <div>
            <h5>3.存在的问题、改进措施及建议 （着重分析教与学中的薄弱环节，并提出改进措施或建议）</h5>
            <div class="answer">
                ${sessionScope.answers[2]}
            </div>
        </div>
        <form method="post" action="/youke/youkeFinalDownload" target="_blank" onsubmit="downloadPaperSubmit()">
            <input type="hidden" name="barBase64Info" id="hidden" value=""/>
            <button type="submit">下载试卷分析报告</button>
        </form>
...
<script type="text/javascript">
    ...
    /*function downloadPaper(){
        document.getElementById('hidden').value = barBase64Infovar;
        alert(document.getElementById('hidden').value);
        var url="/youke/youkeFinalDownload";
        window.open(url);
        /!*!//Ajax发送数据给后台
        $.ajax({
            url :"/youke/youkeFinalDownload",
            cache : true,
            type : "post",
            datatype : "json",
